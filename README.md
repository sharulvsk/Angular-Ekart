...
container.component.html
    ...
    <p>Description : Latest model smartphone</p>
    <p>Discounted Price : {{'$'+getDiscount().toFixed(2)}}</p>
    <p>{{product.inStock > 0 ?'Only '+product.inStock+' items Left':'Out of Stock'}}</p>
    <button (click)="decreaseOnClick()">-</button>
    <span>{{addToCart}}</span>
    <button (click)="increaseOnClick()">+</button>-->
